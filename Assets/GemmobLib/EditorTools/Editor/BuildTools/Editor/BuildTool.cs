...
namespace Gemmob.EditorTools {
	internal class BuildTool {
        ...
        private static string BuildAndroid(BuildConfig buildConfig, string suffix, BuildConfig.Options options, OsBuild osBuild) {
			...
			//if (osBuild != null && !string.IsNullOrEmpty(osBuild.machineName)) {
			//	filename = osBuild.machineName + "_" + filename;
			//}
			...
			Debug.LogFormat("Prepre for build {0}", filename);
			try {
...
#region other
		...
		public static string GetKeyStorePath() {
			return Path.GetDirectoryName(Application.dataPath) + "/" + PlayerSettings.applicationIdentifier +
			       ".keystore";
		}
