...
public class Menu<T> : MonoBehaviour where T : Component {
	...
	private Panel GetPanel<T>() where T : Panel {
		...
		if (p == null) {
			GameObject prefab = Resources.Load<GameObject>(string.Format("{0}{1}/{2}", PrefabResourcesPath, typeof(T).Name, typeof(T).Name));
			if (prefab == null) {
				Debug.LogErrorFormat("[Menu] Can't find prefab at path Resources/{0}{1}/{2}", PrefabResourcesPath, typeof(T).Name, typeof(T).Name);
				return null;
			}
